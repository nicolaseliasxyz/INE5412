...
void PRIORITY::addProcess(std::unique_ptr<Process> process) {
...
bool PRIORITY::run() {
    while (currentProcess != nullptr) {
        ...
        while (!finished) {
            ...
            dispatcher();
            // Zueirinha a mais que percorre a lista apos adicionar processo novo pra alternar
            interruptByPriority();
        }
        yield();
    }
...
bool PRIORITY::compareByPriority(const std::unique_ptr<Process>& a, const std::unique_ptr<Process>& b) {
    return a->getPriority() > b->getPriority();
}

void PRIORITY::interruptByPriority() {
    for (int i = 0; i < processVector.size(); i++) {
        if (processVector[i]->getState() == Process::State::READY && processVector[i]->getPriority() > currentProcess->getPriority()) {
            yield();
        }
    }
}
...
void PRIORITY::init() {
    std::cout << "Escalonamento PRIORITY:" << std::endl;
    dispatcher();
